...
const { joinOfficialPluginName } = require('./resolvePlugin')
const { resolveEntryIndex } = require('./resolveEntryIndex')


const forEachCheckIsTsFile = (paths) => {
  let tsPath = ''
  try {
    Array.isArray(paths) && paths.forEach(_path => {
      if(Array.isArray(_path)) {
        tsPath = forEachCheckIsTsFile(_path)
      }
      if (typeof _path === 'string') {
        const entryFileExt = path.extname(_path)
        tsPath = ['.ts', '.tsx'].includes(entryFileExt.toLowerCase())
      }
      if(tsPath) {
        tsPath = _path
        throw Error()
...
exports.checkTsProjectRun = (service, args) => {
  ...
  entryTsFile = forEachCheckIsTsFile(entryFilePaths)
  if(entryTsFile) {
    const tsName = joinOfficialPluginName('typescript')
    const eslintTsName = joinOfficialPluginName('eslint-typescript')
    if(!service.pkgDepend[tsName] && !service.pkgDepend[eslintTsName]) {
      const tipMessage = ['']
      tipMessage.push(`Entry file ${entryTsFile} is typescript file.`)
      tipMessage.push(`Recommended to install "${tsName}" plugin or "${eslintTsName}" plugin.`)
      tipMessage.push('Ignore the prompt. --no-check-ts-project')
      warn(tipMessage.join('\n'), service.servicePkg.name)
    }
  }
}

exports.checkNodeVersion = (targetVersion) => {
  if (!semver.satisfies(process.version, targetVersion)) {
    error(
      `You are using Node ${process.version}, but fe-cli-service ` +
      `requires Node ${targetVersion}.\nPlease upgrade your Node version.`
    )
    process.exit(1)
  }
}
